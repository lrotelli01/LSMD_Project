...
@Aggregation(pipeline = {
    ...
    "   $cond: { " +
    "       if: { $eq: ['$ratingStats.count', 0] }, " +
    "       then: 0, " +
    "       else: { $divide: ['$ratingStats.sum', '$ratingStats.count'] } " +
    "} } } }",
...
ReservationStatsDTO getReservationStatistics(
...
@Aggregation(pipeline = {
    ...
    "{ '$addFields': { " +
    "   'yearMonth': { $dateToString: { format: '%Y-%m', date: '$creationDate' } } " +
    "} }",
    "{ '$group': { " +
    "   '_id': '$yearMonth', " +
...
List<RatingEvolutionDTO> getRatingEvolution(
...
@Aggregation(pipeline = {
    "{ '$addFields': { " +
    "   'variance': { '$pow': [{ '$stdDevPop': '$reviews.rating' }, 2] } " +
    "} }",
    "{ '$match': { 'variance': { $gt: 0 } } }",
    "{ '$sort': { 'variance': -1 } }",
    "{ '$limit': 10 }",
