...
Finding properties within a specific radius using MongoDB's geospatial capabilities. The implementation uses \texttt{MongoTemplate} for dynamic query construction:
...
public List<PropertyResponseDTO> getPropertiesInArea(double lat, double lon, double radiusKm) {
    ...
    // Uses 'location' field (GeoJSON format) with '2dsphere' index
    query.addCriteria(Criteria.where("location")
            .nearSphere(new Point(lon, lat))
            .maxDistance(radiusMeters / 6378137.0)); // Earth radius in meters
...
\begin{itemize}
    ...
    \item \textbf{nearSphere}: Uses Earth's curvature for accurate distance calculation.
    \item \textbf{MongoTemplate}: Provides flexible, programmatic query construction for complex geospatial queries.
