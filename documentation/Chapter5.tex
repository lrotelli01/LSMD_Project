...
\begin{itemize}
    \item \textbf{Location Hierarchy Compound Index}
    \begin{itemize}
        \item \textbf{Type:} Compound Index
        \item \textbf{Fields:} \texttt{\{ "city": 1, "region": 1, "country": 1 \}}
        \item \textbf{Justification:} Optimizes hierarchical drill-down filtering (e.g., selecting a country, then a region, then a city) without requiring separate indexes for each level.
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.5\textwidth]{images/LHCNoIndex.png}
      \caption{Query results without index}
      \label{fig:my_label}
    \end{figure}
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.5\textwidth]{images/LHCIndex.png}
    ...
    \item \textbf{Rating Sort Compound Index}
    \begin{itemize}
        \item \textbf{Type:} Compound Index
        \item \textbf{Fields:} \texttt{\{ "ratingStats.value": -1 \}}
    ...
    \begin{itemize}
        ...
        \item \textbf{Fields:} \texttt{coordinates}
        \item \textbf{Justification:} Required to support geospatial queries such as \texttt{\$near} and \texttt{\$geoWithin}, satisfying the requirement to view properties on a map or find POIs nearby.
    \begin{figure}[H]
      \centering
...
\begin{itemize}
    \item \textbf{Room Price Compound Index}
    \begin{itemize}
        \item \textbf{Type:} Compound Multikey Index
        \item \textbf{Fields:} \texttt{rooms.pricePerNightAdults} (Asc), \texttt{rooms.pricePerNightChildren} (Asc)
        \item \textbf{Justification:} Optimizes complex budget queries where users filter by both adult and child prices simultaneously. This is more efficient than separate indexes because it allows the database to narrow down candidate rooms satisfying both price constraints in a single index scan.
     \begin{figure}[H]
          \centering
    ...
    \item \textbf{Room Type Index}
    \begin{itemize}
        \item \textbf{Fields:} \texttt{rooms.roomType}
        \item \textbf{Justification:} Optimizes searches where users specify a required room configuration (e.g., filtering for "Single", "Double", or "Suite").
     \begin{figure}[H]
          \centering
          \includegraphics[width=0.5\textwidth]{images/RTNoIndex.png}
          \caption{Query results without index}
          \label{fig:my_label}
        \end{figure}
        \begin{figure}[H]
          \centering
          \includegraphics[width=0.5\textwidth]{images/RTIndex.png}
          \caption{Query results with index}
          \label{fig:my_label}
        \end{figure}
    \end{itemize}

    \item \textbf{Room Beds Index}
    \begin{itemize}
        \item \textbf{Fields:} \texttt{rooms.beds}
        \item \textbf{Justification:} Accelerates queries where users filter by the specific number of beds required (e.g., distinguishing between a double bed room and a twin room with 2 separate beds).
     \begin{figure}[H]
          \centering
          \includegraphics[width=0.5\textwidth]{images/RBNoIndex.png}
          \caption{Query results without index}
          \label{fig:my_label}
        \end{figure}
        \begin{figure}[H]
          \centering
          \includegraphics[width=0.5\textwidth]{images/RBIndex.png}
          \caption{Query results with index}
          \label{fig:my_label}
        \end{figure}
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Justification:} Allows the system (and Managers) to quickly retrieve all reservations associated with a specific room.
        \begin{figure}[H]
      \centering
